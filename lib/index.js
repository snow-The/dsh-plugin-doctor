/**
 * dsh-plugin-doctor — static vetting for DSH plugins.
 *
...
// rule: [id, label, regex, severity(1..3)]
const RULES = [
...
async function scanFile(file, maxBytes = 2 * 1024 * 1024) {
  ...
  // hidden unicode per-char scan (only on text-y files)
  ...
  for (const [id, label, re, sev] of RULES) {
    if (id === 'obf-hidden-unicode') continue // handled above
    for (let i = 0; i < lines.length; i++) {
      re.lastIndex = 0
      let m
      while ((m = re.exec(lines[i])) !== null) {
        const raw = m[0]
        // skip base64 rule hits inside already-flagged lines to reduce noise
        hits.push({ rule: id, severity: sev, evidence: label, line: i + 1, snippet: lines[i].trim().slice(0, 140) })
        if (id === 'obf-base64' && raw.length > 300) break
...
export function apply(ctx) {
  const scanTool = (name) => ({
    ...
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Absolute path to a directory or file to scan (default: current profile node_modules dsh-plugin-* packages)' },
      },
    },
    timeoutMs: 120000,
