...
async function scanFile(file, maxBytes = 2 * 1024 * 1024) {
    const hits = [];
    let text;
    try {
        const st = await stat(file);
        if (st.size > maxBytes)
            return { file, hits: [], note: 'skipped (too large)' };
    ...
    if (isText) {
        ...
        for (let i = 0; i < text.length; i++) {
            if (HIDDEN.has(text.charCodeAt(i)))
                unicode++;
        }
        if (unicode > 0)
            hits.push({ rule: 'obf-hidden-unicode', severity: 2, evidence: `hidden unicode chars: ${unicode}`, line: 0 });
    }
    const lines = text.split(/\r?\n/);
    for (const [id, label, re, sev] of RULES) {
        if (id === 'obf-hidden-unicode')
            continue;
        for (let i = 0; i < lines.length; i++) {
            ...
            while ((m = re.exec(lines[i])) !== null) {
                ...
                if (hits.filter((h) => h.rule === id && h.line === i + 1).length > 3)
                    break;
            }
        }
    }
...
async function scanTarget(root) {
    ...
    if (packageJson) {
        try {
            const pj = JSON.parse(await readFile(packageJson, 'utf8'));
...
export function apply(ctx) {
    const scanTool = (toolName) => ({
        name: toolName,
        description: 'Static vetting scan of a plugin directory or file. Never executes scanned code. Returns per-file evidence (file:line, rule, severity), a verdict (SAFE/LOW/MEDIUM/HIGH) and any lifecycle scripts found. Path must be an absolute path on this machine.',
        parameters: {
            type: 'object',
            properties: {
        ...
        async execute(args) {
            ...
            return { ok: true, ...res, summary };
        },
    });
    for (const name of ['doctor_scan', 'doctor_scan_path']) {
        try {
            ctx.tools.register(scanTool(name));
        }
        catch (err) {
